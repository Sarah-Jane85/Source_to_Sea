...
def prep_choropleth_data(engine) -> pd.DataFrame:
    """Load emissions from SQL, join country names, aggregate by country."""
    df_raw = pd.read_sql("""
        SELECT e.emission_tons_year, c.country_name, c.iso_code
        FROM emission_points e
        JOIN countries c ON e.country_id = c.country_id
        WHERE e.emission_tons_year > 0
    """, engine)
    return (df_raw.groupby(['country_name', 'iso_code'])['emission_tons_year']
                  .sum().reset_index()
                  .rename(columns={'emission_tons_year': 'total_emissions'}))
...
def plot_emission_distribution(engine) -> None:
    ...
    df = pd.read_sql("SELECT emission_tons_year FROM emission_points WHERE emission_tons_year > 0", engine)

    fig = px.histogram(df, x="emission_tons_year", nbins=50, log_y=True,
                       title="Distribution of River Plastic Emissions (Univariate)",
                       labels={"emission_tons_year": "Emission (tons/year)"},
                       color_discrete_sequence=["#1f77b4"])
    
    fig.update_layout(showlegend=False)
    fig.show()

    df_zoom = df[df['emission_tons_year'] < 1000]
    fig2 = px.histogram(df_zoom, x="emission_tons_year", nbins=50,
...
def plot_emissions_by_income_group(engine) -> None:
    ...
    df = pd.read_sql("""
        SELECT emission_tons_year, income_group
        FROM emission_points
        WHERE income_group IS NOT NULL AND emission_tons_year > 0
    """, engine)
    fig = px.box(df, x="income_group", y="emission_tons_year", log_y=True,
                 title="Plastic Emissions by Country Income Group (Bivariate)",
                 labels={"income_group": "Income Group", "emission_tons_year": "Emission (tons/year)"},
